...
const getCoinList = (page, currecy) =>
  `${BASE_URL}/coins/markets?vs_currency=${currecy}&order=market_cap_desc&per_page=20&page=${page}&x_cg_demo_api_key=${API_KEY}`;

const searchCoin = (query) =>
  `${BASE_URL}/search?query=${query}&x_cg_demo_api_key=${API_KEY}`;

const marketChart = (coins) =>
  `${BASE_URL}/coins/${coins}/market_chart?vs_currency=usd&days=7`;
